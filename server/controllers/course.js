...
export const checkEnrollment = async (req,res) => {
    ...
    //check if course id is found in the user course array
    let ids= []
    let length = user.courses && user.courses.length
    for (let i = 0; i < length; i++) {
        ids.push(user.courses[i].toString())
        console.log('IDS CONSOLED ====>', ids)
    }
    res.json({
...
export const paidEnrollment = async (req,res) => {
  try {
    ...
    res.send(session.id);
      ...
      console.log('HANDLE PAIDED ENROLLEMTN', err)
      return res.status(400).send('Enrollment create failed')
  }
...
export const markCompleted =async (req, res) => {
    ...
    // find if user that course is already created
...
//                message:'Todo not found with id' + req.parms.courseId
//            })
//        }
...
//            message:'To do not found with id' + req.params.courseId
//        })
//    })
// }
